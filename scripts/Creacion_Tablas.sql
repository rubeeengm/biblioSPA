...
CREATE TABLE IF NOT EXISTS `PRESTAMO_LIBROS`.`LIBROSDEALUMNO` ( 
    ...
    PRIMARY KEY (`ID`),
    UNIQUE (`IDLIBRO`),
    FOREIGN KEY FK_LIBROSDEUSUARIO_IDALUMNO(`IDALUMNO`) REFERENCES `ALUMNOS`(`ID`),
    FOREIGN KEY FK_LIBROSDEUSUARIO_IDLIBRO(`IDLIBRO`) REFERENCES `LIBROS`(`ID`) ON DELETE CASCADE
);